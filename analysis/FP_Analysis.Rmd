...
table <- function(tuples) {
  kable(custom_summary(tuples), "html", caption = "Custom Summary of Recursive Tuples") %>%
...
table(tuples_clean_rec)
table(tuples_clean_DP)
table(tuples_clean_memo)
```
...
```{r, echo=TRUE, results='show'}
ggplot(combined_tuples, aes(x = Type, y = Capacity, fill = Type)) +
  ...
  theme_minimal() +
  labs(title = "Average Capacity by Algorithm Type with Data Points", 
       x = "Algorithm Type", y = "Average Capacity") +
...
```{r, echo=TRUE, results='show'}
# Plot for Recursive
ggplot(tuples %>% filter(Type == "Recursive"), aes(x = Time, y = Capacity)) +
  geom_point(color = "royalblue2") +
  theme_minimal() +
...
# Relationship:
#
# Linear relationship between DP Execution Time and the Capacity
# EQN: a * (Capacity) + b
#
lm_dp <- lm(Time ~ Capacity, data = tuples_clean_DP)

intercept_dp <- coef(lm_dp)[1]
slope_dp <- coef(lm_dp)[2]
r_squared_dp <- summary(lm_dp)$r.squared

reg_eq_dp <- paste("y = ", round(slope_dp, 5), " * Capacity - ", (-1) * round(intercept_dp, 2), 
                     "\n", "R² = ", 1 - round(r_squared_dp, 3))

ggplot(tuples %>% filter(Type == "DP"), aes(x = Capacity, y = Time)) +
  geom_point(color = "tomato") +  
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "brown") +  
  theme_minimal() +
  labs(title = "DP Runtime vs Capacity with Linear Regression", 
       x = "Capacity", y = "DP Runtime") +
  theme(plot.title = element_text(hjust = 0.5)) +
  annotate("text", x = max(tuples_clean_DP$Capacity) * 0.5, 
           y = max(tuples_clean_DP$Time) * 0.9,
           label = reg_eq_dp, color = "black", size = 5, hjust = 0)
# ----------- DP -----------
```
...
# Relationship:
#
# Linear relationship between Memo Execution Time and the Capacity
# EQN: a * (Capacity) + b
#
lm_memo <- lm(Time ~ Capacity, data = tuples_clean_memo)

intercept_memo <- coef(lm_memo)[1]
slope_memo <- coef(lm_memo)[2]
r_squared_memo <- summary(lm_memo)$r.squared

reg_eq_memo <- paste("y = ", round(slope_memo, 2), " * Capacity + ", round(intercept_memo, 2), 
                     "\n", "R² = ", 1 - round(r_squared_memo, 3))

ggplot(tuples %>% filter(Type == "Memo"), aes(x = Capacity, y = Time)) +
  geom_point(color = "springgreen3") +  # Plot the data points
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "brown") + 
  theme_minimal() +
  labs(title = "Memo Runtime vs Capacity with Linear Regression", 
       x = "Capacity", y = "Memo Runtime") +
  theme(plot.title = element_text(hjust = 0.5)) +
  annotate("text", x = max(tuples_clean_memo$Capacity) * 0.5, 
           y = max(tuples_clean_memo$Time) * 0.9,
           label = reg_eq_memo, color = "black", size = 5, hjust = 0)
# ---------- Memo ----------
```
